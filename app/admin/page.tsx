      ...
      {selectedInvoice && (
        ...
        >
          <div className="p-8">
            ...
            {/* ── SIGNED DECLARATION (print-mode: declaration / invoice-declaration / full-package) ── */}
            {(signatureDataUrl || signerName) && (
              <div data-print-mode="declaration" className="mt-8 border-2 border-slate-900 p-6 break-inside-avoid">
            ...
            {/* ── COVER LETTER (print-mode: cover-letter / full-package) ── */}
            <div data-print-mode="cover-letter" className="mt-10 pt-8 border-t-2 border-slate-900">
              <div className="flex flex-col sm:flex-row justify-between items-start gap-6 mb-8">
                <div>
                  ...
                  <div className="text-xs text-slate-500 font-mono mt-1 space-y-0.5">
                    ...
                    <div>{SITE_CONFIG.whatsappFormatted}</div>
                ...
                <div className="text-right text-xs text-slate-500 font-mono">
                  ...
                  <div className="mt-2">Date: {new Date().toLocaleDateString("en-ZA", { day: "2-digit", month: "long", year: "numeric" })}</div>
                </div>
              </div>

              <div className="mb-6">
                <div className="text-xs uppercase font-bold text-slate-500 mb-1">To:</div>
              ...
              <div className="text-sm text-slate-800 leading-relaxed space-y-4">
                <p>Dear {selectedInvoice.clientName.split(" ")[0]},</p>

                <p>
                  Thank you for considering {SITE_CONFIG.tradingName} for your project. Please find attached your
                  {selectedInvoice.documentType === "Quote" ? " proposal / quotation" : " invoice"} for
                  <strong className="text-slate-900"> {selectedInvoice.invoiceNumber}</strong>.
                </p>

                {selectedInvoice.documentType === "Quote" && selectedInvoice.proposalSummary && (
                  <p>
                    <strong>Project scope:</strong> {selectedInvoice.proposalSummary}
                  </p>
                )}

                <p>
                  <strong>Total value:</strong> {selectedInvoice.currency === "ZAR" ? "R" : "$"} {calculateInvoiceSubtotal(selectedInvoice).toLocaleString()}
                  {selectedInvoice.documentType === "Quote" && (
                    <> (Kick-off deposit: {selectedInvoice.currency === "ZAR" ? "R" : "$"} {calculateDepositAmount(selectedInvoice).toLocaleString()})</>
                  )}
                </p>

                <p>
                  {selectedInvoice.documentType === "Quote"
                    ? "This proposal includes a 48-hour staging demo so you can see the build in action before committing. I look forward to working together."
                    : "Payment is due by the date shown on the invoice. Thank you for your business."
                  }
                </p>

                <p>Kind regards,<br /><strong>{SITE_CONFIG.developerName}</strong><br />{SITE_CONFIG.tradingName}<br />{SITE_CONFIG.whatsappFormatted}</p>
              </div>
            </div>
          </div>
        </div>
