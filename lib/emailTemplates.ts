...
import { buildInviteCard } from "./clientPortal";
...
export function buildHandoverEmailDraft(inv: InvoiceLike): string {
  return [
    ...
    ``,
    `Every project includes a 14-day bug-fix warranty: critical bugs found after handover are fixed at no charge (Terms, clause 10.2). Optional care plans are available if you'd like ongoing maintenance.`,
    ``,
...
export function buildSignRequestEmailDraft(inv: InvoiceLike): string {
  ...
  return [
    `Hi ${inv.clientName},`,
    ``,
    `Please find your ${docTitle} ${inv.invoiceNumber} attached as a PDF.`,
    ``,
    `To sign it:`,
    `1. Open the attached PDF.`,
    `2. Sign the declaration on the first page — your full name and signature where marked.`,
    `3. Return the signed PDF by replying to this email, or WhatsApp it to ${SITE_CONFIG.whatsappFormatted}.`,
    ``,
...
export function buildCPInviteMessage(account: ClientPortalAccount, channel: "email" | "whatsapp" = "email"): string {
  ...
  const body = [
    ...
    ``,
    `Track your build live with % progress, view your quote, check payments, read my updates and reply, open your staging links, and sign your declaration — all in one place, no admin dashboard needed.`,
    ``,
    `${divider}`,
    `🔗 PORTAL: ${portalUrl()}`,
