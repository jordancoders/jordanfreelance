...
export function buildQuoteEmailDraft(inv: InvoiceLike): string {
  ...
  body.push(
    ``,
    `Payment is via PayPal.me (${SITE_CONFIG.paypalMeUrl}) or Direct EFT (bank transfer) — proof of payment via WhatsApp ${SITE_CONFIG.whatsappFormatted}.`,
...
export function buildHandoverEmailDraft(inv: InvoiceLike): string {
  return [
    ...
    ``,
    `Final balance due: ${money(inv, balanceOf(inv))} — payable via PayPal.me or Direct EFT.`,
    `PayPal.me: ${SITE_CONFIG.paypalMeUrl}`,
    ``,
